...
// 初始化选项
function uss_get_default_options()
{
    return [
        'bucket' => '',
        'OperatorName' => '',
...
{
    add_option('uss_options', uss_get_default_options(), '', 'yes');
}

function uss_get_client()
{
    $uss_opt = get_option('uss_options', uss_get_default_options());
...
{
    $uss_options = get_option('uss_options', uss_get_default_options());
        ...
        if ($link) {
            if ($upload_path != '.') {
                ...
                if (count($file_info) >= 2) {
                    $file = $upload_path . end($file_info);
                }
            } else {
                $uss_options = get_option('uss_options', uss_get_default_options());
...
{
    $option = get_option('uss_options', uss_get_default_options());
...
function uss_setting_page()
{
    ...
    $check_nothumb = $uss_nothumb == 'true' ? 'checked="checked"' : '';

    $uss_nolocalsaving = esc_attr($uss_options['nolocalsaving']);
    $check_nolocalsaving = $uss_nolocalsaving == 'true' ? 'checked="checked"' : '';
                ...
                <tr>
                    ...
                    <td>
                      <select name="update_file_name">
                        <option <?php echo $uss_update_file_name == 'false' ? 'selected="selected"' : '';?> value="false">不处理</option>
                        <option <?php echo $uss_update_file_name == 'md5' ? 'selected="selected"' : '';?> value="md5">MD5</option>
                        <option <?php echo $uss_update_file_name == 'time' ? 'selected="selected"' : '';?> value="time">时间戳+随机数</option>
                      </select>
                    </td>
                </tr>
                <tr>
